...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
    ...
    @Query("SELECT COUNT(t) FROM Transaction t WHERE t.user.id = :userId AND t.category IS NULL")
    Long countUncategorizedByUserId(@Param("userId") Long userId);

    @Query("SELECT SUM(t.amount) FROM Transaction t WHERE t.user.id = :userId AND t.amount > 0 AND t.operationTime BETWEEN :start AND :end")
    ...
    @Query("SELECT t FROM Transaction t WHERE t.user.id = :userId ORDER BY t.operationTime DESC LIMIT :limit")
